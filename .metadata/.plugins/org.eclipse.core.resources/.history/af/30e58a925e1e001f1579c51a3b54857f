...
public class Client implements ClientService, ProtocolImpl {
	...
	// Clinet의 화면 부분의 컴포넌트를 멤버변수로 가져와 담을 변수
	...
	// JOptionPane 사용할 때 알침창의 아이콘 변결할 이미지
	...
	public Client() {
		...
		MsgBtn = clientFrame.getWaitingRoomPanel().getMsgBtn();
		sendMessageBtn = clientFrame.getMessagePanel().getMessageBtn();
	}

	// 소켓연결 하는 connectNerwork(), 입출력 장치를 연결하는 connectIO()
	...
	public void clickConnectServerBhn(String ip, int port, String id) {
		...
		try {
			...
			writer.flush();
			clientFrame.setTitle("[Talk_" + id + "님 ]");

			clientFrame.getIndexPanel().getLoginBtn().setEnabled(false);
	...
	// 서버측의 요청을 받아올 Reader
	// 요청을 받아 msg에 저장하고, checkProtocol() 호출한다.
	...
	// protocol -> 어떤 작업을 해야 하는지 알려주는 약속
	// from -> 보내는 측이 담겨 있다
	...
	// 클라이언트 화면단에서 정보를 받아오는 인터페이스
	@Override
	public void clickMessageBhn(String messageText) {
	...
	@Override
	public void clickMakeRoom(String roomName) {
		writer("MakeRoom/" + roomName);

	}

	@Override
	public void clickOutRoom(String roomName) {
		writer("OutRoom/" + roomName);

	}

	@Override
	public void clickEnterRoom(String roomName) {
