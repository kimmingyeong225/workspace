...
public class MultiThreadServer {
	...
	public static void main(String[] args) {
		...
		try (ServerSocket serverSocket = new ServerSocket(5000)) {
			...
			PrintWriter writerStream = 
					new PrintWriter(socket.getOutputStream(), true);// true -> (오토 + 역할을 해줌)
			
			// 키보드 스트림 준비
			BufferedReader keyboardReader = 
					new BufferedReader(new InputStreamReader(System.in)); // 키보드 -> System.in
	...
	private static void startReadThread(BufferedReader bufferedReader) {
		...
		// 메인 스레드 대기 처리 --> join() --> 고민 --> 2번에 반복 될 듯??
